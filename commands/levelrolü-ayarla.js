    ...
    async execute(message, args) {
        ...
        if (!args.length) {
            let embed = new Discord.MessageEmbed()
                ...
                .addFields({ name: `${prefix}role-level ekleme <level> <@role>`, value: `Belirli bir seviyeye yükseldiklerinde kullanıcıya verilecek bir rol belirler..` })
                .addFields({ name: `${prefix}role-level kaldırma <level>`, value: `Belirtilen seviyedeki rol seviyesini kaldırır.` })
                .addFields({ name: `${prefix}rol seviyesi gösterme`, value: `Seviyelere ayarlanmış tüm rolleri gösterir.` })
                .setColor("RANDOM");
