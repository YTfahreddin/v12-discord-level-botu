    ...
    if (talkedRecently.get(message.author.id)) {
        ...
        if (level.xp >= nextXP) {
            ...
            let embed = new Discord.MessageEmbed()
                ...
                .setDescription(`**Tebrikler** ${message.author}! Şimdi seviye atladınız **level ${level.level}**`)
                .setColor("RANDOM")
                .setThumbnail(message.author.displayAvatarURL({ dynamic: true }))
                .setTimestamp();
            // using try catch if bot have perms to send EMBED_LINKS      
            try {
                client.channels.cache.get('log kanal idi').send(embed)
            } catch (err) {
                message.channel.send(`Tebrikler, ${message.author}! Şimdi seviye atladınız **Level ${level.level}**`)
            }
        };
